...
export const RecipeProvider = ({children}) => {
    ...
    recipes.io.add = async(data) => {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                let res = await fetch(`${BACKEND_URI}/api/recipes`, {
                    "method": "POST",
                    "headers": {
    ...
    recipes.io.delete = async(data) => {
        return new Promise(async(resolve, reject) => {
            try {
                const token = await session.getToken()
                await fetch(`${BACKEND_URI}/api/recipes/${data.idx}`, { method: 'DELETE', "headers": { "Authorization": `Bearer ${token}`} }).then(
                    (res) => res.json()
                )
                recent.get()
                recent.getForUser() 
    ...
    recipes.io.attachImage = async(image) => {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                let res = await fetch(`${BACKEND_URI}/api/recipes/images/upload`, {
                    "method": "POST",
                    "headers": {
                        "Authorization": `Bearer ${token}`
                    },
    ...
    recipes.io.getSubmissionStatus = async(idx) => {
        return new Promise(async(resolve, reject) => {
            try {
                const token = await session.getToken()
                let res = await fetch(`${BACKEND_URI}/api/submissions/${idx}`, {
                    "method": "GET",
                    "headers": {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${token}`
    ...
    recipes.io.updateImage = async(image, idx) => { 
        return new Promise(async(resolve, reject) => {
            try {
                ...
                const formData = new FormData()
                formData.append("image", image)
                let res = await fetch(`${BACKEND_URI}/api/recipes/images/upload/${idx}`, {
                    "method": "PUT",
                    "headers": {
                        "Authorization": `Bearer ${token}`
                    },
    ...
    recipes.recent.get = async(skip = 0, limit = config.pageLength.current, updateState = true) => {
        return new Promise(async(resolve, reject) => {
            try {
                const result = await fetch(`${BACKEND_URI}/api/recipes/${skip}/${limit}`).then((raw) => raw.json())
    ...
    recipes.recent.getForUser = async(skip = 0, limit = config.pageLength.current, updateState = true) => {
        return new Promise(async(resolve, reject) => {
            try {
                const token = await session.getToken()
                const result = await fetch(`${BACKEND_URI}/api/recipes/user/${skip}/${limit}`, 
    ...
    recipes.search.query = async(query, skip = 0, limit = config.pageLength.current, updateState = true) => {
        return new Promise(async(resolve, reject) => {
            try {
                ...
                const result = await fetch(`${BACKEND_URI}/api/search/${skip}/${limit}?q=${query}`).then((rawData) => rawData.json())
    ...
    recipes.specific.get = async(idx) => {
        return new Promise(async(resolve, reject) => {
            try {
                const result = await fetch(`${BACKEND_URI}/api/recipes/${idx}`).then((rawData) => rawData.json())
                setSpecific(result)
                resolve(result)
            } catch (e) {
                alert('Could not load recipes, please check your internet connection and refresh the page')
