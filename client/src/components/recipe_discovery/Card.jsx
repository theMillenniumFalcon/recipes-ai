...
export default function Card(props) {
    if (props.name) return (
        <Link to={`/recipe/view/${props.obj._id}`}>
            <div className={`font-poppins rounded-lg bg-white shadow-shadow-1 flex md:flex-col w-full h-full`}>
                ...
                <div className="w-[70%] md:w-full flex flex-col p-3 md:px-4 md:pt-0 md:mt-5 grow">
                    ...
                    overflow-hidden text-blue-1 md:text-lg line-clamp-2 capitalize">
                        <span>{props.name}</span>
                    </div>
                    <div className="grow hidden md:block"></div>
                    <div className="flex flex-col md:flex-row">
                        <span className="mt-4 md:mt-0 font-normal text-blue-2 text-sm truncate grow">
                            {props.chef}
                        </span>
    ...
    return (
        <Link to="#">
            <div className={`font-poppins rounded-lg bg-white shadow-shadow-1 card`} 
            style={{width: props.width, height: props.height}}>
                <div className="bg-slate-200 animate-pulse bg-cover h-[150px] w-[90%] relative left-[5%] top-[5%] rounded-[5px]"></div>
                <div className="font-poppins font-semibold text-lg h-[60px] w-[90%] ml-[5%] mt-[20px] 
                text-ellipsis overflow-hidden text-blue-1">
                    <div className="mt-1 h-7 w-52 rounded-lg bg-slate-200 animate-pulse"></div>
                </div>
                <div className="bg-slate-200 animate-pulse float-left relative top-2 left-[5%] h-5 w-28 rounded-md"></div>
                <div className="bg-slate-200 animate-pulse float-right relative right-[5%] top-2 h-5 w-20 rounded-md"></div>
            </div>
        </Link>
    )
}