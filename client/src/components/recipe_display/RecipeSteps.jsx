...
export default function RecipeSteps({ currentRecipe }) {
    ...
    return (
        <>
            <section className='flex flex-col gap-x-6 gap-y-5 text-blue-1 font-poppins'>
                {!isLoaded &&
                    <div className='bg-slate-200 animate-pulse h-24 w-full rounded-xl'/>
                }
                {isLoaded &&
                    currentRecipe.steps?.map((step, index) => (
                        <div className='flex flex-col bg-white shadow-shadow-2 rounded-[30px]'>
                            ...
                            <div className='bg-white shadow-shadow-1 py-5 px-3 rounded-[30px] flex items-center gap-3 border-s-2 border-blue-1 grow'>
                                <div className='bg-white relative t-[50%] font-medium text-2xl shadow-shadow-1 border-2 border-[#63b890]
